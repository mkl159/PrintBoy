...
{
    ui_text_center(ui, ui->font_medium, "CONTROLE DU JOB", 40, 640, C_ACCENT);

    int y = 80;
    for (int i = 0; i < NUM_ACTIONS; i++) {
        SDL_Color bg = (i == ui->cursor) ? C_ACCENT : C_PANEL;
        SDL_Color fg = (i == ui->cursor) ? C_BG : C_FG;
        ui_box(ui, 60, y, 520, 44, bg);
        ui_text(ui, ui->font_medium, labels[i], 80, y + 8, fg);
        y += 54;
    }

    ui_text_center(ui, ui->font_small,
        "Haut/Bas : choisir   A : executer", 420, 640, C_DIM);
}

static void do_action(ui_t *ui, int idx)
{
    pb_config_t *c = ui->cfg;
